...
# world. the function slam takes fiveparameters as input 
# and returns the vector mu. This vector should have x, y 
# coordinates interlaced, so for example, 
...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    dim = 2 * (N+ num_landmarks) #each point has a x,y coord
    
    #Build the constraint matrix/vector
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1.0
    Omega.value[1][1] = 1.0
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            for b in range(2):
                ...
                Xi.value[n+b][0]      += measurement[i][b+1] / measurement_noise
                Xi.value[m+b][0]      += measurement[i][b+1] / measurement_noise
    ...
    # compute the best estimate of the robot location
    mu = Omega.inverse() * Xi
